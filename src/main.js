...
// Функция для отрисовки фильтра
const getFilter = (type, count, status = ``) => `<input
  type="radio"
  id="filter__${type}"
...
const filters = filtersData
  .map((it) => getFilter(it.type, it.count, it.status))
  .join(``);
filterBlock.insertAdjacentHTML(`beforeend`, filters);

// Функция для отрисовки карточки
const getTask = () => `<article class="card card--blue">
